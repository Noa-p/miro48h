...
const initPlugin = async () => {
  ...
  await miro.board.setAppData('456', {
    bob: 'alice'
  });
  console.log(await miro.board.getAppData('456')); // setTimeout(async () => {
  ...
  */
  ...
  Event.sub(Event.type.SelectFrames, async items => {
    for (const frame of items) {
      console.log(frame.id); // console.log(await miro.board.getAppData(frame.id))
